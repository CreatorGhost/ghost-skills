...
2. **Write a RED test that reproduces the risk.** Before (or without) the fix,
...
3. **Drive the real artifact.** Call the actual exported function / real endpoint
   / shipped binary with real (or faithfully recorded) inputs. No reconstructed
   payloads, schemas, or SQL.
4. **Assert the content contract, not liveness.** Assert the *value/effect*: the
   ...
   assertion.
5. **Cover the edges, boundaries, and negatives.** Empty, null, single, huge,
...
6. **Add at least one real-I/O / contract test at the boundary you don't
...
7. **Hunt the silent failure.** If the code swallows errors, falls back to a
...
8. **Verify in the SHIPPED artifact + telemetry.** Confirm the fix is in the
...
A test can be green for all the wrong reasons. Each is detailed in
`references/02-false-green-smells.md`.
...
- `references/01-principles.md` — the testing pyramid; behavior-vs-implementation;
  ...
  cardinal rule.
- `references/02-false-green-smells.md` — the full anti-pattern catalogue: each
  smell, why it lies, a generalized example (passing→fixed), and the fix.
- `references/03-coverage-checklist.md` — a fill-in checklist for any change:
  happy path, edges, negatives/errors, boundaries & limits, encodings/i18n,
  concurrency/idempotency, the dependency contract, performance/size caps, and
  the "did I assert the actual contract?" gate.
- `references/04-verification-gate.md` — the "BEFORE you say it's fixed" gate
  (saw red → real artifact → content contract → real dependency once → shipped
  build → telemetry → failure-path did-not-fire).
